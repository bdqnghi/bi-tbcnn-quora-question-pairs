...
def init_net_for_bi_tbcnn(feature_size):
    ...
    with tf.name_scope("network"):
        conv1 = conv_layer(1, 300, nodes, children, feature_size)
...
def hidden_layer(pooled, input_size, output_size):
    ...
    with tf.name_scope("hidden"):
        ...
        return lrelu(tf.matmul(pooled, weights) + biases, 0.01), weights
...
def contrastive_loss(model1, model2, y, margin):
    ...
    with tf.name_scope("contrastive-loss"):
        ...
        tmp2 = (1 - y) * tf.square(tf.maximum((margin - distance),0))
        return tf.reduce_mean(tmp + tmp2) /2, distance
